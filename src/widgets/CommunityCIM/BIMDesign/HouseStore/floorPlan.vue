<template>
  <a-carousel dots-class="slick-dots slick-thumb" arrows>
    ...
    <div v-for="item in statictisImgArr" :key="item">
...
const baseUrl = "/agsupport-rest/agsupport/BIM/Project/get";
export default {
  ...
  mounted() {
    if (this.selectBinPropety)
      this.findThumb(this.baseUrl, this.selectBinPropety);
  },
  watch: {
    selectBinPropety(newV, oldV) {
      this.findThumb(this.baseUrl, newV);
    },
  },
  methods: {
    getImgUrl(i) {
    ...
    async findThumb(url, newV) {
      let _url = this.baseUrl;
      this.beferPath = newV.beferPath;
      let da = await axiosWraper.getData(_url,{paramType: 1, id: newV.id});
      if (da.success) {
