...
let currentBuildingTileset = null;
let currentCommunity = null;
let currentCommunityLG = null;
export default {
  components: {
  ...
  methods: {
    ...
    stateFromChild(changestate) {
      ...
      if (changestate == "isBuilding") {
        viewer.zoomTo(currentBuildingTileset, viewAngle);
      }
    },
    gotoCommunity() {
      var _this = this;
      ...
      agFeatureLayer.removeAll();
      var tileset;
      tileset = CIM.layerTree.getLayerById(
        "8ad45762-d0d5-4e97-bf33-c60ff33ff93c"
      )._primitives[0];
      tileset.readyPromise.then(function(argument) {
        currentBuildingTileset = tileset;
      });
      viewer.scene.globe.depthTestAgainstTerrain = false; //关闭地形检测
      ...
      currentCommunity = agFeatureLayer.addPrimitive(newPrimitive);
      currentCommunityLG = agFeatureLayer.addPrimitive(newLGPrimitive);
      currentCommunity.setID = "currentCommunity";
      currentCommunityLG.setID = "currentCommunityLG";

      agRenderer.setMaterialFromImage(currentCommunity, resultimg);
      agRenderer.setMaterialFromImage(currentCommunityLG, resultLGimg);
    },
  },
  destroyed() {
    viewer.scene.globe.depthTestAgainstTerrain = true; //开启地形检测
