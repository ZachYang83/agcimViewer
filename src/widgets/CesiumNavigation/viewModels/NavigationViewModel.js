...
NavigationViewModel.prototype.show = function (container) {
  ...
  if (this.enableZoomControls && this.enableCompass) {
    testing = '<div class="compass" title="" data-bind="visible: showCompass, event: {dblclick: handleDoubleClick }">' +
      ...
      ' <div class="compass-gyro-background" data-bind="visible: showCompass, event: { mousedown: handleMouseDown }"></div>' +
...
NavigationViewModel.prototype.handleMouseDown = function (viewModel, e) {
  ...
  if (e.button == 0) {
    //得到当前屏幕中心坐标
    var car = CIM.viewer.camera.pickEllipsoid(new Cesium.Cartesian2(CIM.viewer.canvas.clientWidth / 2, CIM.viewer.canvas.clientHeight / 2));

    //转换高度
    var pos = coordinate.changePointHeight(car, CIM.viewer.camera.positionCartographic.height + 1000)

    //飞到此中心点俯视图位置，如果直接设置，可以将flyTo改成setView
    CIM.viewer.camera.flyTo({
      destination: pos,
      orientation: {
        heading: CIM.viewer.camera.heading,
