class ColorPicker {
	...
	init(opt) {
		let { el, offset, initColor = "rgb(255,0,0)", allMode = ['hex', 'rgb'], color = '' } = opt;
		...
		this.elem_okBtn = div.getElementsByClassName("colorpicker-okBtn")[0];
		this.elem_cancelBtn = div.getElementsByClassName("colorpicker-cancelBtn")[0];
		...
		this.bindElem.addEventListener("click", function () {
			...
			if (!offset) offset = 0;
			top = h - top - 88 < 180 ? top - 88 - 180 : top - offset;

			_this.pancelLeft = left + 180;
			...
			_this.css(div, {
				...
				"display": 'none',
				"left": left + "px",
				"top": top + _this.bindElem.offsetHeight + "px"
			});

			// _this.bindMove(_this.elem_colorPancel, _this.setPosition, true);
			// _this.bindMove(_this.elem_barPicker1.parentNode, _this.setBar, false);
		...
		this.elem_okBtn.addEventListener("click", function (e) {
			if (_this.rgba) {
		...
		this.elem_cancelBtn.addEventListener("click", function (e) {
			_this.hide();
		}, false);
	...
	render() {
		var tpl =
			`<div style="position: fixed; top: 0px; right: 0px; bottom: 0px; left: 1000px;"></div>
	...
	setPancelColor(h) {
		...
		this.css(this.elem_colorPancel, {
	...
	bindMove(elem, fn, bool) {
		...
		elem.addEventListener("mousedown", function (e) {
			...
			function mouseup(e) {
				...
				_this.elem_inputWrap.innerHTML = _this.getInputTpl()
			}
		}, false);
	}
	show() {
		this.css(this.elem_wrap, {
			"display": "block"
		})
	}
	hide() {
