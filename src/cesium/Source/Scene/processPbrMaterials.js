...
function generateTechnique(
  ...
  vertexShader += "bool isPointInBound(vec2 point, vec4 bounds){\n",
    vertexShader += "   return (point.x>bounds.x&&point.x<bounds.z&&point.y<bounds.w&&point.y>bounds.y);",
    vertexShader += "}\n",
    vertexShader += "float unpackDepth(const in vec4 rgba_depth){\n",
    vertexShader += "   const vec4 bitShifts = vec4(1.0, 1.0 / 255.0, 1.0 / (255.0 * 255.0), 1.0 / (255.0 * 255.0 * 255.0));\n",
    vertexShader += "   float depth=dot(rgba_depth, bitShifts);\n",
    vertexShader += "   return depth;\n",
    vertexShader += "}\n";
  var ceslab = gltf.asset && gltf.asset.generator && "cesiumlab" == gltf.asset.generator;
  if (ceslab) {
    vertexShader += "uniform bool IsYaPing;\n",
      vertexShader += "uniform sampler2D u_polygonTexture;\n",
      vertexShader += "uniform vec4 u_polygonBounds[20];\n",
      vertexShader += "uniform float u_height[20];\n";
    for (var i = 0; i < 10; i++) {
      vertexShader += "uniform vec4 u_polygonBounds" + i + ";\n",
        vertexShader += "uniform float u_height" + i + ";\n";
    }
  }
  //源码修改结束
  ...
  //源码修改开始 （地形压平)
  if (ceslab) {
    vertexShaderMain += "if(IsYaPing){\n";
    for (var i = 0; i < 10; i++) {
      vertexShaderMain += "   if (isPointInBound(weightedPosition.xy, u_polygonBounds" + i + ")){\n",
        vertexShaderMain += "        vec2 texCoord;\n",
        vertexShaderMain += "        texCoord.x = (weightedPosition.x-u_polygonBounds" + i + ".x)/(u_polygonBounds" + i + ".z-u_polygonBounds" + i + ".x);\n",
        vertexShaderMain += "        texCoord.y = (weightedPosition.y-u_polygonBounds" + i + ".y)/(u_polygonBounds" + i + ".w-u_polygonBounds" + i + ".y);\n",
        ...
        vertexShaderMain += "            weightedPosition.z = u_height" + i + ";\n",
