...
///////////////////////////////////////////////////////////////////////////
//新增，用于获取3dtiles的顶点数据-蔡周峻
Model.vertexs = [];
Model.prototype.getVertexs = function () {
  let gltf = this.gltf;
  let classifyVertexs = [[]];
  try {
    for (let meshIndex = 0; meshIndex < gltf.meshes.length; meshIndex++) {
      ...
      for (let primitiveIndex = 0; primitiveIndex < mesh.primitives.length; primitiveIndex++) {
        ...
        parsePrimitive(primitive, classifyVertexs, this);
      }
    }
  } catch (error) {
  ...
  return classifyVertexs;
}
function parsePrimitive(primitive, classifyVertexs, thisModel) {
  let gltf = thisModel.gltf;
  let gltfBuffer;
  let vertexAccessor;
  let bachIdAccessor;
  try {
    gltfBuffer = gltf.buffers[0].extras._pipeline.source;
    vertexAccessor = gltf.accessors[primitive.attributes["POSITION"]];
    bachIdAccessor = gltf.accessors[primitive.attributes["_BATCHID"]];
    let vertexs = parseVertex(vertexAccessor, gltf, gltfBuffer, thisModel.modelMatrix);
    let ids = parseBatchId(bachIdAccessor, gltf, gltfBuffer);

    for (let vertexsIndex = 0; vertexsIndex < vertexs.length; vertexsIndex++) {
      const vertex = vertexs[vertexsIndex];
      let batchIdIndex = ids[vertexsIndex];
      if (classifyVertexs.length < batchIdIndex + 1) {
        let indexOffset = batchIdIndex + 1 - classifyVertexs.length;
        for (let j = 0; j < indexOffset; j++) {
          classifyVertexs.push([]);
        }
      }
      classifyVertexs[batchIdIndex].push(vertex);
    }
  } catch (error) {
...
function parseVertex(vertexAcc, gltf, gltfBuffer, modelMatrix) {
  let bufferId = vertexAcc.bufferView;
  let bufferView = gltf.bufferViews[bufferId];
  let startIndex = bufferView.byteOffset + vertexAcc.byteOffset;
  let lenght = bufferView.byteLength;
  let vertexBuffer = gltfBuffer.slice(startIndex, startIndex + lenght);
  let stride = (bufferView.byteStride > 0 ? bufferView.byteStride : 12);
  let arrayBuffer = new Uint8Array(gltfBuffer).buffer;
  let vertexs = [];
  let typedView = new Float32Array(arrayBuffer, startIndex, vertexAcc.count * 3);

  for (let j = 0; j < typedView.length - 2; j += 3) {
    let position = new Cartesian3(typedView[j], typedView[j + 1], typedView[j + 2]);
    var worldPosition = Matrix4.multiplyByPoint(modelMatrix, position, new Cartesian3());
    vertexs.push(worldPosition);
  }
  return vertexs;
}

function parseBatchId(bachIdAcc, gltf, gltfBuffer) {
  let bufferId = bachIdAcc.bufferView;
  let bufferView = gltf.bufferViews[bufferId];
  let startIndex = bufferView.byteOffset + bachIdAcc.byteOffset;
  let lenght = bufferView.byteLength;
  let batchIdBuffer = gltfBuffer.slice(startIndex, startIndex + lenght);
  let stride = (bufferView.byteStride > 0 ? bufferView.byteStride : 12);
  let arrayBuffer = new Uint8Array(gltfBuffer).buffer;
  let batchId = [];
  let typedView = new Int16Array(arrayBuffer, startIndex, bachIdAcc.count);

  for (let j = 0; j < typedView.length; j++) {
    batchId.push(typedView[j]);
  }
  return batchId;
}
  ...
  for (var i = 0; i < length; ++i) {
    ...
    uniformMap = combine(uniformMap, {
      ...
      u_polygonTexture: function () {
        return flat.tilesEditor.fbo && flat.tilesEditor.fbo._colorTextures[0] || context._defaultTexture
      },
      u_polygonBounds0: function () {
        return flat.tilesEditor.polygonBounds[0] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds1: function () {
        return flat.tilesEditor.polygonBounds[1] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds2: function () {
        return flat.tilesEditor.polygonBounds[2] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds3: function () {
        return flat.tilesEditor.polygonBounds[3] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds4: function () {
        return flat.tilesEditor.polygonBounds[4] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds5: function () {
        return flat.tilesEditor.polygonBounds[5] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds6: function () {
        return flat.tilesEditor.polygonBounds[6] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds7: function () {
        return flat.tilesEditor.polygonBounds[7] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds8: function () {
        return flat.tilesEditor.polygonBounds[8] || new Cartesian4(0, 0, 0, 0)
      },
      u_polygonBounds9: function () {
        return flat.tilesEditor.polygonBounds[9] || new Cartesian4(0, 0, 0, 0)
      },
      u_height0: function () {
        return flat.tilesEditor.height[0] || 0
      },
      u_height1: function () {
        return flat.tilesEditor.height[1] || 0
      },
      u_height2: function () {
        return flat.tilesEditor.height[2] || 0
      },
      u_height3: function () {
        return flat.tilesEditor.height[3] || 0
      },
      u_height4: function () {
        return flat.tilesEditor.height[4] || 0
      },
      u_height5: function () {
        return flat.tilesEditor.height[5] || 0
      },
      u_height6: function () {
        return flat.tilesEditor.height[6] || 0
      },
      u_height7: function () {
        return flat.tilesEditor.height[7] || 0
      },
      u_height8: function () {
        return flat.tilesEditor.height[8] || 0
      },
      u_height9: function () {
        return flat.tilesEditor.height[9] || 0
      },
      IsYaPing: function () {
        return flat.tilesEditor.IsYaPing
      },
      //源码修改结束
    });
...
Model.prototype.update = function (frameState) {
  ...
  if (defined(loadResources) && this._state === ModelState.LOADED) {
    ...
    if (loadResources.finished()) {
      ...
      //获取顶点数据，可能对加载速度有所影响-蔡周峻
      /*  if (this.vertexs == null || this.vertexs.length == 0 || this.vertexs == undefined) {
